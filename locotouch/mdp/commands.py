...
class WarmupUniformVelocityCommand(UniformVelocityCommand):
    def __init__(self, cfg: WarmupUniformVelocityCommandCfg, env: ManagerBasedEnv | ManagerBasedRLEnv):
        super().__init__(cfg, env)
        self.initial_zero_command_steps = self.cfg.initial_zero_command_steps
        self.metrics["initial_zero_command_steps"] = (
            torch.ones(self.num_envs, device=self.device) * self.initial_zero_command_steps
        )
    ...
    def _update_command(self):
        ...
        if self.initial_zero_command_steps <= 0:
            return
        zero_command_env_ids = (self._env.episode_length_buf < self.initial_zero_command_steps).nonzero(
            as_tuple=False
        ).flatten()
        if zero_command_env_ids.numel() > 0:
            self.vel_command_b[zero_command_env_ids, :] = 0.0


@configclass
class WarmupUniformVelocityCommandCfg(UniformVelocityCommandCfg):
    class_type: type = WarmupUniformVelocityCommand
    initial_zero_command_steps: int = 0


class WarmupUniformVelocityCommandMultiSampling(WarmupUniformVelocityCommand):
    def __init__(self, cfg: WarmupUniformVelocityCommandMultiSamplingCfg, env: ManagerBasedEnv | ManagerBasedRLEnv):
        super().__init__(cfg, env)
        ...
        self.sampling_probs = torch.tensor(
            [self.cfg.new_command_probs, 1.0 - 2 * self.cfg.new_command_probs, self.cfg.new_command_probs],
            device=self.device,
        )
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        if self.binary_maximal_command:
            sampling_indices = torch.randint(0, self.maximal_command_sampling.shape[0], (len(env_ids),), device=self.device)
            maximal_commands = torch.tensor(
                [self.cfg.ranges.lin_vel_x[1], self.cfg.ranges.lin_vel_y[1], self.cfg.ranges.ang_vel_z[1]],
                device=self.device,
            )
            self.vel_command_b[env_ids] = self.maximal_command_sampling[sampling_indices] * maximal_commands
        else:
            if self.lin_vel_x_equal_ranges and self.lin_vel_y_equal_ranges and self.ang_vel_z_equal_ranges:
                ...
                if self.lin_vel_x_equal_ranges:
                    ...
                    for i, (low, high) in enumerate(self.lin_vel_x_sampling_ranges):
                        ...
                        if in_env_ids.any():
                            ...
                            self.vel_command_b[selected_envs, 0] = torch.empty(
                                len(selected_envs), device=self.device
                            ).uniform_(low, high)
                if self.lin_vel_y_equal_ranges:
                    ...
                    for i, (low, high) in enumerate(self.lin_vel_y_sampling_ranges):
                        ...
                        if in_env_ids.any():
                            selected_envs = env_ids[torch.nonzero(in_env_ids, as_tuple=True)[0]]
                            self.vel_command_b[selected_envs, 1] = torch.empty(
                                len(selected_envs), device=self.device
                            ).uniform_(low, high)
                if self.ang_vel_z_equal_ranges:
                    ...
                    for i, (low, high) in enumerate(self.ang_vel_z_sampling_ranges):
                        ...
                        if in_env_ids.any():
                            selected_envs = env_ids[torch.nonzero(in_env_ids, as_tuple=True)[0]]
                            self.vel_command_b[selected_envs, 2] = torch.empty(
                                len(selected_envs), device=self.device
                            ).uniform_(low, high)
...
@configclass
class WarmupUniformVelocityCommandMultiSamplingCfg(WarmupUniformVelocityCommandCfg):
    class_type: type = WarmupUniformVelocityCommandMultiSampling
